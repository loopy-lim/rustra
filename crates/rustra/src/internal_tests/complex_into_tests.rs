...
// (frame_wire.rs 의 Status fixture 와 동일한 라우팅)
...
fn complex_command_gets_into_handler_and_writes_caller_buffer() {
    ...
    match pkg.invoke_frame_into(&req, &mut target).unwrap() {
    ...
    let buffered = pkg.invoke_frame(&req).unwrap();
...
// (trust_baseline_ffi::caller_buffer_frame_large_response_executes_exactly_once)
...
fn complex_into_fallback_executes_handler_exactly_once() {
    ...
    assert!(matches!(
        pkg.invoke_frame_into(&req, &mut roomy).unwrap(),
