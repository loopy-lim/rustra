impl Package {
    #[inline]
    fn invoke_frame_command(&self, command: &Command, payload: &[u8]) -> crate::Result<Vec<u8>> {
        // Runtime Authority: deny-by-default — capability 가 요구되는데 부여되지
        // 않았으면 바이너리 핸들러(또는 JSON fallback)를 호출하지 않고
        // capability.denied 를 반환한다. 에러는 Frame error wire 로 인코딩되어
        ...
        // 계약으로, 패닉을 `internal` 에러로 정규화해 Frame 에러 프레임으로 반환한다.
        ...
        let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            with_state_context(&self.states, || {
                ...
                if let Some(ref handler) = command.frame_handler {
                    return handler(payload);
                }

                // Fallback: legacy JSON-based path for commands without binary handler
                if !command.frame_tier3 && payload.len() < 8 {
    ...
    /// Frame caller-buffer 경로. 정적 postcard command는 호스트가 제공한
    /// slice에 직접 응답을 기록해 Rust heap allocation과 memcpy를 없앤다.
    pub fn invoke_frame_into(
        &self,
        payload: &[u8],
        target: &mut [u8],
        ...
        if self.is_frozen() {
            ...
            return self.invoke_frame_into_command(command, payload, target);
        }
        ...
        self.invoke_frame_into_command(command.as_ref(), payload, target)
    }

    fn invoke_frame_into_command(
        &self,
        command: &Command,
        payload: &[u8],
        target: &mut [u8],
    ) -> crate::Result<DirectResponse> {
        let Some(handler) = command.frame_into_handler.as_ref() else {
            return self
                .invoke_frame_command(command, payload)
