...
**엔진 정체성.** 엔진은 값이 아니라 객체 정체성(`WeakMap`)으로 구분한다. `RustraProvider` 에는
모듈 상수나 `useMemo` 결과처럼 안정적인 엔진을 전달한다. 렌더링마다 새 객체가 만들어지는
불안정한 `engine` prop 은 엔진 교체로 관측되어 매 렌더링 `useMutation` 상태를 초기화한다.
의도한 교체에서 상태가 초기화되는 것은 설계된 동작이다.
