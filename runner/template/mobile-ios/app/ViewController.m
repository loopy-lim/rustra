...
// bundle 내부 App.tsx 가 greet({name:"rustra"}) rkyv 호출 → 결과 "Hello, rustra!" 렌더링.
// 스파이크 examples/lynx-calculator/ios/app/ViewController.m 에서 정제 추출.
...
- (void)viewDidLoad {
  ...
  NSLog(@"[template-ios] module: RustraModule registered");
  ...
  NSLog(@"[template-ios] loadTemplate path=%@ bytes=%lu", path, (unsigned long)template.length);
