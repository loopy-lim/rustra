...
- `PackageBuilder::function`·`try_function`으로 매크로 없이 일반 Rust 함수를 등록한다.
  arity 마킹 어댑터가 기존 핸들러·디스패치 가드를 재사용하고, 0~12 인자와 일반
  반환값을 지원하며, 함수 메타데이터(`functionArgs`)로 위치 인자 TS 시그니처를
  생성한다. 레거시 명령의 와이어·스키마·계약 해시는 불변이다.

### Changed

- 검증된 동기 바인딩(`bindSync`)과 호출·코덱 최적화를 추가한다. 생성된
  바인딩이 자기 명령의 속성 이름을 설치 시점에 소유하는 bound codec context를
  쓰며, 공개 동기 호출의 소형 인자 경로가 8–16% 빨라졌다(A/B 수화물 참고).
- 작은 맵 인코딩이 항목 수와 무관한 임시 vector 대신 4개 이하 고정 저장소를 쓴다.
