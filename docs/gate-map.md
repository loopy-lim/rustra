...
| `rust-wasm32`    | `rustra` compiles for `wasm32-unknown-unknown`                                                                                                                                                                                         | not required (see drift note below)                                                      | `cargo check -p rustra --target wasm32-unknown-unknown`                                             |
...
- The `gate` aggregate job is **not** a required check (confirmed by the same
  ...
  Step 3.5.
- Drift note: `release-procedure.md` Step 3.5 still lists 9 contexts (including
  `rust-wasm32`); the live list as of 2026-09-20 is the 8 above. This document
  is the live-state reference; the procedure doc had not caught up.
