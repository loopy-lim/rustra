...
1 MiB다. Rust 발송 쪽에서 페이로드 한도(기본 1 MiB, `rustra_ffi_set_max_payload` 로
조정)를 넘는 프레임은 채널 `send`/`send_bytes` 가 `false` 를 반환하게 만든다 —
발송 측 코드는 반환값을 반드시 검사해야 하며, invoke 의 `payload.too_large`
거부와 같은 계열이다. JS 재조립의 상한은 16 MiB이며 미완성 메시지는 30초 후 만료한다. JSON은 최종 재조립 후 한 번만
