...
bun run --cwd examples/calculator codegen                # render TS surfaces from it
```
...
<!-- prettier-ignore -->
```ts
export const GENERATED_CONTRACT_HASH = 'e70c24c62944ff78fb6ca58b5658d85bb5cff49af4acc96b3a78bedcbdfb0a58';
...
- Shown truncated to the first command; the real file carries all 32 commands.
- Four fields appear only when declared, so undeclared packages keep byte-identical schemas: per-command `errors` (`[{ code, description, retryable }]`, from `#[command(error(...))]` → `errors.ts`), `devices` (`["camera", ...]`, from `#[command(device(...))]` → `devices.ts`), `platforms` (from `.platform_command`), and the top-level `deviceCapabilities` catalog the CLI reads for the `devices.ts` union and the `codegen.device_catalog` doctor check. See [docs/internal/codegen.md](internal/codegen.md) for the full shape.
...
Events on Node: when the schema declares events, the generated `node.ts` entry also
exports `subscribeEvent` (push over stdout frames when the loop-stdio runtime accepts the
`events:"push"` handshake, polling fallback otherwise). Every host's subscription and
channel surface is in [events-and-channels.md](events-and-channels.md).
...
While prototyping on a debug build you can call a Rust handler that is not in the
generated client yet with `invokeLoose(engine, 'commandName', args?)` from
`@rustra/types` — the dynamic development tier, its unknown-device-token wall, and the
release-time gates are described in [dev-tier.md](dev-tier.md).
