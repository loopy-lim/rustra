...
후속 릴리스 진행: 사용자가 PR #69와 커밋·병합·발행 진행을 확인했다.
감사 수정 8개 커밋은 `41619bb7`부터 `8079239d`까지 생성했고, Rust 0.10.0 및
각 JS 패키지의 다음 minor 버전을 적용했다. 아래 내용은 버전 적용 전 감사 검증
기록이다. 최종 커밋의 CI·레지스트리 결과는 PR과 릴리스 실행 기록에서 확인한다.
