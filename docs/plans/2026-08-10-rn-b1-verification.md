...
- [ ] `examples/react-native-calculator` 에서 iOS 대상 Rust 빌드 스크립트 실행
      (`build-rust-ios.sh` / `cargo build --target x86_64-apple-ios-sim` 등 사전 구성).
- [ ] `expo run:ios` (또는 `npx react-native run-ios`) — **C++ 코덱이 정상 링크**되는지. - `rustra-codec.hpp/.cpp`, `rustra-generated-codecs.{hpp,cpp}` 가 Xcode 타겟에 포함되는지. - `RustraJSIBridge.cpp` 의 신규 심볼(`hasStaticCodec`, `invokeTyped`, `invokeTypedBatch`)이
      호스트 객체에 노출되는지(`globalThis.__rustraNative` 에 존재).
- [ ] 앱 실행 시 JSI install 성공 로그.

## 2. 단건 호출 정확성 (C++ fast path)

`invokeTyped` 경로(정적 명령)가 JS 코덱과 동일한 결과를 반환하는지. 각 명령을 한 번씩:

- [ ] `addNumbers({a, b})` → `value` 일치.
- [ ] `multiply({a, b})` → `value` 일치.
- [ ] `isEven({n})` → `result` 일치.
- [ ] `sumList({numbers})` → `count`/`total` 일치 (vec 경로).
- [ ] `createItem({name, value})` → 중첩 `item` 객체 일치 (struct 경로).
- [ ] `processItem({item})` → 중첩 입력 + 중첩 출력 일치.
- [ ] `greet({name})` / `toUpper({s})` → string UTF-8(멀티바이트/이모지 포함) 일치.
- [ ] `rustraRegistryDemo({op})` → **`ok`/`frozen`/`message` 가 올바른 값**에 매핑되는지
...
- [ ] 런타임 등록 동적 명령(`ping`/`average`/`greetDyn` 등) 호출 → 정상 결과.
- [ ] 결과가 JSON 디코딩으로 복원됨(문자열/배열/중첩 객체).

## 4. invokeBatch (P0-2)

- [ ] `invokeBatch([{command:'addNumbers',...},{command:'multiply',...}])` →
      결과가 **순서대로** 반환되고 각 값이 단건 호출과 일치.
- [ ] 단일 JSI 횡단으로 처리되는지 확인(가능하면 `invokeTypedBatch` 호출 카운트 로그).
- [ ] 정적 + 동적 혼합 배치 → 항목별 라우팅(typed/Tier3)으로 폴백, 결과 순서 유지.
- [ ] 배치 중 한 항목이 Rust 에러 → 전체 배치가 해당 에러로 reject(fail-fast).
...
- [ ] Android 대상 Rust 빌드 (`cargo build --target aarch64-linux-android` 등).
- [ ] `expo run:android` (또는 `npx react-native run-android`) — C++ 코덱이 NDK 로 동일 동작.
- [ ] §2 단건 + §3 동적 + §4 배치를 Android 에서도 재확인 (C++ 코드는 공통).

## 7. 회귀 / 호환성

- [ ] 기존 raw `invokeRkyvV2(ArrayBuffer)` 경로(레거시/벤치마크)가 여전히 동작.
- [ ] Node/Bun/Tauri 어댑터(`@rustra/node`, `@rustra/bun`, `@rustra/tauri`) —
