...
| [마이그레이션 노트](migrations/0.3-to-0.4.md), [0.5→0.6](migrations/0.5-to-0.6.md), [post-0.9 Frame·감사](migrations/post-0.9-frame-and-audit.md) | 사용자 | rustra 마이너 버전을 건너뛸 때의 단계별 노트                                                                                                       |
| [Rust API 가이드](rust-api-guide.md)                                                                                                              | 사용자 | `#[command]`/`#[bridge_type]`/`build!` 매크로, Package/Builder API                                                                                 |
| [동기 명령 바인딩](synchronous-bindings.ko.md) ([English](synchronous-bindings.md))                                                               | 사용자 | `bindSync` 실행 자격, 동결 레지스트리·codec 식별 정보, 수명 주기, 오류·한도                                                                        |
| [에러 코드](error-codes.ko.md) ([English](error-codes.md))                                                                                        | 전체   | `RustraErrorCode` 전체 표 — 코드, 발급 계층, retryable, JSON 폴백 동작, TS↔Rust 동기화 게이트                                                      |
| [일반 함수 등록](function-registration.ko.md) ([English](function-registration.md))                                                               | 사용자 | `PackageBuilder::function`/`try_function` 으로 매크로 없이 일반 Rust 함수 연결 — 0~12 인자, 위치 인자 TS 클라이언트                                |
...
| [릴리즈 매트릭스](release-matrix.ko.md) ([English](release-matrix.md))                                                                            | 기여자 | 타깃별 릴리즈 검증 매트릭스 — 무엇을 어디서 검증하는지                                                                                             |
| [게이트 지도](gate-map.ko.md) ([English](gate-map.md))                                                                                            | 기여자 | 모든 게이트의 단일 지도 — 보호 불변식, 로컬 명령, CI 잡, required-check 여부                                                                       |
| [API 서피스 게이트](../api-surface/README.md)                                                                                                     | 기여자 | 스냅샷 비교 절차, `--update` 규칙, TS/Rust 경계 스코프                                                                                             |
...
`docs/api/`는 생성 TypeDoc HTML(`bun run docs:api`)이다 — 손글 문서가 아닌 로컬 빌드 산출물이다.
