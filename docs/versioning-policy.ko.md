...
| `rustra_ffi_capture_snapshot` | 실험        | B1 인스펙터 덤프. 스냅샷 blob 형태(필드, JSON 레이아웃)는 예고 없이 바뀔 수 있다.           |
| `@rustra/types` inspector     | 실험        | `DumpedWire` 타입과 `parseSnapshot`/`serializeSnapshot` 은 위 blob 계약을 함께 따른다.      |
