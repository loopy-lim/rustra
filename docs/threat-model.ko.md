...
| A3  | 와이어 프로토콜(Frame / JSON 프레임)                                          | 파서 결함 = Rust의 메모리 안전 결함                              |
...
3. **Rust → JS 이벤트 푸시**(`events.rs`, Tauri `emit`, RN
   ...
   모든 `rustra://` 이벤트를 수신한다.
4. **역직렬화기**(`frame_decode.rs`, 어댑터의 JSON 경로). 정의상 신뢰하지
   않는 입력 — Frame 디코더는 수동 작성된 경계 검사 커서 디코더다(모든
...
- **FFI의 신뢰하지 않는 JS 입력**: 크기 게이트 → 레지스트리 조회 → 스키마
  ...
  `errors.ts`).
- **역직렬화(Frame / JSON)**: Frame 디코드는 전면 수동·경계 검사;
  JSON 경로는 어댑터에 있으며 표준 파서와 에러 프레임용
  `parseRustraErrorString`을 지난다. Frame 경로가 fuzz 대상; JSON 파싱
