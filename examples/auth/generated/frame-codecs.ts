...
export const adminStatsCodec: FrameCodec<AdminStatsInput, AdminStatsOutput> = {
  ...
  decode(buf: ArrayBuffer | ArrayBufferView): { ok: boolean; result?: AdminStatsOutput; error?: RustraError } {
...
export const grantCodec: FrameCodec<GrantInput, GrantOutput> = {
  ...
  decode(buf: ArrayBuffer | ArrayBufferView): { ok: boolean; result?: GrantOutput; error?: RustraError } {
    // caller-buffer 뷰(Uint8Array subarray 등)도 받는다 — node-loop 가 왕복당
    // 사본 없이 프레임 뷰를 그대로 넘긴다. DataView 는 ArrayBuffer 만 받으므로
    // (buf.buffer, byteOffset) 로 정규화한다.
    const isView = ArrayBuffer.isView(buf);
...
export const signInCodec: FrameCodec<SignInInput, SignInOutput> = {
  ...
  decode(buf: ArrayBuffer | ArrayBufferView): { ok: boolean; result?: SignInOutput; error?: RustraError } {
    // caller-buffer 뷰(Uint8Array subarray 등)도 받는다 — node-loop 가 왕복당
    // 사본 없이 프레임 뷰를 그대로 넘긴다. DataView 는 ArrayBuffer 만 받으므로
    // (buf.buffer, byteOffset) 로 정규화한다.
    const isView = ArrayBuffer.isView(buf);
...
export const signOutCodec: FrameCodec<SignOutInput, SignOutOutput> = {
  ...
  decode(buf: ArrayBuffer | ArrayBufferView): { ok: boolean; result?: SignOutOutput; error?: RustraError } {
    // caller-buffer 뷰(Uint8Array subarray 등)도 받는다 — node-loop 가 왕복당
    // 사본 없이 프레임 뷰를 그대로 넘긴다. DataView 는 ArrayBuffer 만 받으므로
    // (buf.buffer, byteOffset) 로 정규화한다.
    const isView = ArrayBuffer.isView(buf);
