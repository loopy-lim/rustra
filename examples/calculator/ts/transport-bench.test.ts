...
describe('transport performance', { concurrency: 1 }, () => {
  ...
  if (existsSync(napiPath)) {
    ...
    it('napi-rs is faster than subprocess', () => {
      ...
      console.log(
        `    subprocess: ${subR.avg.toFixed(0)}ns  napi-rs: ${napiR.avg.toFixed(0)}ns  ratio: ${(subR.avg / napiR.avg).toFixed(1)}x`,
      );

      if (NAPI_FASTER_THAN_SUBPROCESS) {
        assert(
          napiR.avg < subR.avg,
          `napi-rs (${napiR.avg.toFixed(0)}ns) should be faster than subprocess (${subR.avg.toFixed(0)}ns)`,
        );
      }
    });
  } else {
