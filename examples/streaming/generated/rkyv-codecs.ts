...
export const jobStatusCodec: RkyvV2Codec<JobStatusInput, JobStatusOutput> = {
  commandId: 2,
  ...
  encode(args: JobStatusInput): ArrayBuffer {
    ...
    parts.push(cmdId);
    parts.push(_pcEncodeString(args.jobId));
    return _pcConcatUint8Arrays(parts).buffer as ArrayBuffer;
  },

  encodeInto(args: JobStatusInput, reuse?: Uint8Array): Uint8Array {
    ...
    out[w++] = 2; out[w++] = 0;
    { const _s = args.jobId; const _u = _utf8Encode(_s); ensure(5 + _u.length); let _v = _u.length; do { out[w++] = (_v % 128) | 0x80; _v = Math.floor(_v / 128); } while (_v > 0); out[w - 1] &= 0x7f; out.set(_u, w); w += _u.length; }
  ...
  decode(buf: ArrayBuffer): { ok: boolean; result?: JobStatusOutput; error?: RustraError } {
    ...
    let offset = 8;
    const result: Partial<JobStatusOutput> = {};
    {
      const _v = _pcDecodeVarint64(u8, offset);
      result.pendingEvents = _pcDecodeZigzag64(_v.value);
      offset += _v.bytesRead;
    }
    {
      const _v = _pcDecodeVarint64(u8, offset);
      result.droppedEvents = _pcDecodeZigzag64(_v.value);
      offset += _v.bytesRead;
    }
    return { ok: true, result: result as JobStatusOutput };
  },
};

export const startJobCodec: RkyvV2Codec<StartJobInput, StartJobOutput> = {
  commandId: 1,
  ...
  encode(args: StartJobInput): ArrayBuffer {
    ...
    parts.push(cmdId);
    parts.push(_pcEncodeString(args.jobId));
    parts.push(_pcEncodeZigzag64(args.totalSteps));
    parts.push(_pcEncodeZigzag64(args.stepDelayMs));
    return _pcConcatUint8Arrays(parts).buffer as ArrayBuffer;
  },

  encodeInto(args: StartJobInput, reuse?: Uint8Array): Uint8Array {
    let out = reuse ?? new Uint8Array(64);
    let w = 0;
    const ensure = (need: number) => {
      if (w + need <= out.length) return;
    ...
    out[w++] = 1; out[w++] = 0;
    { const _s = args.jobId; const _u = _utf8Encode(_s); ensure(5 + _u.length); let _v = _u.length; do { out[w++] = (_v % 128) | 0x80; _v = Math.floor(_v / 128); } while (_v > 0); out[w - 1] &= 0x7f; out.set(_u, w); w += _u.length; }
    { let _x = BigInt(args.totalSteps); if (_x < _pcI64Min || _x > _pcI64Max) throw new Error('zigzag64 input outside i64 range: ' + _x.toString()); _x = (_x << 1n) ^ (_x >> 63n); do { ensure(1); out[w++] = Number(_x & 0x7fn) | 0x80; _x >>= 7n; } while (_x !== 0n); out[w - 1] &= 0x7f; }
  ...
  decode(buf: ArrayBuffer): { ok: boolean; result?: StartJobOutput; error?: RustraError } {
    ...
    // Decode postcard from offset 8
    let offset = 8;
    const result: Partial<StartJobOutput> = {};
    {
      result.accepted = u8[offset] === 1;
      offset += 1;
    }
    return { ok: true, result: result as StartJobOutput };
  },
};
