...
export const deleteItemCodec: FrameCodec<DeleteItemInput, DeleteItemOutput> = {
  ...
  decode(buf: ArrayBuffer | ArrayBufferView): { ok: boolean; result?: DeleteItemOutput; error?: RustraError } {
    // caller-buffer 뷰(Uint8Array subarray 등)도 받는다 — node-loop 가 왕복당
    // 사본 없이 프레임 뷰를 그대로 넘긴다. DataView 는 ArrayBuffer 만 받으므로
    // (buf.buffer, byteOffset) 로 정규화한다.
    const isView = ArrayBuffer.isView(buf);
...
export const getItemCodec: FrameCodec<GetItemInput, GetItemOutput> = {
  ...
  decode(buf: ArrayBuffer | ArrayBufferView): { ok: boolean; result?: GetItemOutput; error?: RustraError } {
    // caller-buffer 뷰(Uint8Array subarray 등)도 받는다 — node-loop 가 왕복당
    // 사본 없이 프레임 뷰를 그대로 넘긴다. DataView 는 ArrayBuffer 만 받으므로
    // (buf.buffer, byteOffset) 로 정규화한다.
    const isView = ArrayBuffer.isView(buf);
...
export const listItemsCodec: FrameCodec<ListItemsInput, ListItemsOutput> = {
  ...
  decode(buf: ArrayBuffer | ArrayBufferView): { ok: boolean; result?: ListItemsOutput; error?: RustraError } {
    // caller-buffer 뷰(Uint8Array subarray 등)도 받는다 — node-loop 가 왕복당
    // 사본 없이 프레임 뷰를 그대로 넘긴다. DataView 는 ArrayBuffer 만 받으므로
    // (buf.buffer, byteOffset) 로 정규화한다.
    const isView = ArrayBuffer.isView(buf);
...
export const updateItemCodec: FrameCodec<UpdateItemInput, UpdateItemOutput> = {
  ...
  decode(buf: ArrayBuffer | ArrayBufferView): { ok: boolean; result?: UpdateItemOutput; error?: RustraError } {
    // caller-buffer 뷰(Uint8Array subarray 등)도 받는다 — node-loop 가 왕복당
    // 사본 없이 프레임 뷰를 그대로 넘긴다. DataView 는 ArrayBuffer 만 받으므로
    // (buf.buffer, byteOffset) 로 정규화한다.
    const isView = ArrayBuffer.isView(buf);
